...
# Root
set(JOLT_PHYSICS_ROOT ${PHYSICS_REPO_ROOT})
...
# Create Jolt lib
add_library(Jolt INTERFACE ${JOLT_PHYSICS_SRC_FILES})
...
if (NOT CPP_EXCEPTIONS_ENABLED)
	target_compile_definitions(Jolt INTERFACE $<$<BOOL:${MSVC}>:_HAS_EXCEPTIONS=0>)
endif()

# Set the debug/non-debug build flags
target_compile_definitions(Jolt INTERFACE "$<$<CONFIG:Debug>:_DEBUG>")
target_compile_definitions(Jolt INTERFACE "$<$<CONFIG:Release,Distribution,ReleaseASAN,ReleaseUBSAN,ReleaseTSAN,ReleaseCoverage>:NDEBUG>")

# ASAN and TSAN should use the default allocators
target_compile_definitions(Jolt INTERFACE "$<$<CONFIG:ReleaseASAN,ReleaseTSAN>:JPH_DISABLE_TEMP_ALLOCATOR;JPH_DISABLE_CUSTOM_ALLOCATOR>")

# Setting floating point exceptions
if (FLOATING_POINT_EXCEPTIONS_ENABLED AND "${CMAKE_CXX_COMPILER_ID}" STREQUAL "MSVC")
	target_compile_definitions(Jolt INTERFACE "$<$<CONFIG:Debug,Release>:JPH_FLOATING_POINT_EXCEPTIONS_ENABLED>")
endif()

# Setting the disable custom allocator flag
if (DISABLE_CUSTOM_ALLOCATOR)
	target_compile_definitions(Jolt INTERFACE JPH_DISABLE_CUSTOM_ALLOCATOR)
endif()

# Setting enable asserts flag
if (USE_ASSERTS)
	target_compile_definitions(Jolt INTERFACE JPH_ENABLE_ASSERTS)
endif()

# Setting double precision flag
if (DOUBLE_PRECISION)
	target_compile_definitions(Jolt INTERFACE JPH_DOUBLE_PRECISION)
endif()

# Setting to attempt cross platform determinism
if (CROSS_PLATFORM_DETERMINISTIC)
	target_compile_definitions(Jolt INTERFACE JPH_CROSS_PLATFORM_DETERMINISTIC)
endif()

# Setting to determine number of bits in ObjectLayer
if (OBJECT_LAYER_BITS)
	target_compile_definitions(Jolt INTERFACE JPH_OBJECT_LAYER_BITS=${OBJECT_LAYER_BITS})
endif()

if (USE_STD_VECTOR)
	target_compile_definitions(Jolt INTERFACE JPH_USE_STD_VECTOR)
endif()
...
if (TRACK_BROADPHASE_STATS)
	target_compile_definitions(Jolt INTERFACE JPH_TRACK_BROADPHASE_STATS)
endif()
...
if (TRACK_NARROWPHASE_STATS)
	target_compile_definitions(Jolt INTERFACE JPH_TRACK_NARROWPHASE_STATS)
endif()

# Enable the debug renderer
if (DEBUG_RENDERER_IN_DISTRIBUTION)
	target_compile_definitions(Jolt INTERFACE "JPH_DEBUG_RENDERER")
elseif (DEBUG_RENDERER_IN_DEBUG_AND_RELEASE)
	target_compile_definitions(Jolt INTERFACE "$<$<CONFIG:Debug,Release,ReleaseASAN,ReleaseUBSAN,ReleaseTSAN>:JPH_DEBUG_RENDERER>")
endif()

# Enable the profiler
if (PROFILER_IN_DISTRIBUTION)
	target_compile_definitions(Jolt INTERFACE "JPH_PROFILE_ENABLED")
elseif (PROFILER_IN_DEBUG_AND_RELEASE)
	target_compile_definitions(Jolt INTERFACE "$<$<CONFIG:Debug,Release,ReleaseASAN,ReleaseUBSAN,ReleaseTSAN>:JPH_PROFILE_ENABLED>")
endif()

# Compile the ObjectStream class and RTTI attribute information
if (ENABLE_OBJECT_STREAM)
	target_compile_definitions(Jolt INTERFACE JPH_OBJECT_STREAM)
endif()
...
function(EMIT_X86_INSTRUCTION_SET_DEFINITIONS)
	if (USE_AVX512)
		target_compile_definitions(Jolt INTERFACE JPH_USE_AVX512)
	endif()
	if (USE_AVX2)
		target_compile_definitions(Jolt INTERFACE JPH_USE_AVX2)
	endif()
	if (USE_AVX)
		target_compile_definitions(Jolt INTERFACE JPH_USE_AVX)
	endif()
	if (USE_SSE4_1)
		target_compile_definitions(Jolt INTERFACE JPH_USE_SSE4_1)
	endif()
	if (USE_SSE4_2)
		target_compile_definitions(Jolt INTERFACE JPH_USE_SSE4_2)
	endif()
	if (USE_LZCNT)
		target_compile_definitions(Jolt INTERFACE JPH_USE_LZCNT)
	endif()
	if (USE_TZCNT)
		target_compile_definitions(Jolt INTERFACE JPH_USE_TZCNT)
	endif()
	if (USE_F16C)
		target_compile_definitions(Jolt INTERFACE JPH_USE_F16C)
	endif()
	if (USE_FMADD AND NOT CROSS_PLATFORM_DETERMINISTIC)
		target_compile_definitions(Jolt INTERFACE JPH_USE_FMADD)
	endif()
endfunction()
...
if ("${CMAKE_CXX_COMPILER_ID}" STREQUAL "MSVC")
	if ("${CMAKE_VS_PLATFORM_NAME}" STREQUAL "x86" OR "${CMAKE_VS_PLATFORM_NAME}" STREQUAL "x64")
		if (USE_AVX512)
			target_compile_options(Jolt INTERFACE /arch:AVX512)
		elseif (USE_AVX2)
			target_compile_options(Jolt INTERFACE /arch:AVX2)
		elseif (USE_AVX)
			target_compile_options(Jolt INTERFACE /arch:AVX)
		endif()
	...
	elseif (EMSCRIPTEN)
		if (USE_WASM_SIMD)
			...
			target_compile_options(Jolt INTERFACE -msimd128 -msse4.2)
		endif()
	elseif ("${CMAKE_SYSTEM_PROCESSOR}" STREQUAL "x86_64" OR "${CMAKE_SYSTEM_PROCESSOR}" STREQUAL "AMD64" OR "${CMAKE_SYSTEM_PROCESSOR}" STREQUAL "x86" OR "${CMAKE_SYSTEM_PROCESSOR}" STREQUAL "i386")
		...
		if (USE_AVX512)
			target_compile_options(Jolt INTERFACE -mavx512f -mavx512vl -mavx512dq -mavx2 -mbmi -mpopcnt -mlzcnt -mf16c)
		elseif (USE_AVX2)
			target_compile_options(Jolt INTERFACE -mavx2 -mbmi -mpopcnt -mlzcnt -mf16c)
		elseif (USE_AVX)
			target_compile_options(Jolt INTERFACE -mavx -mpopcnt)
		elseif (USE_SSE4_2)
			target_compile_options(Jolt INTERFACE -msse4.2 -mpopcnt)
		elseif (USE_SSE4_1)
			target_compile_options(Jolt INTERFACE -msse4.1)
		else()
			target_compile_options(Jolt INTERFACE -msse2)
		endif()
		if (USE_LZCNT)
			target_compile_options(Jolt INTERFACE -mlzcnt)
		endif()
		if (USE_TZCNT)
			target_compile_options(Jolt INTERFACE -mbmi)
		endif()
		if (USE_F16C)
			target_compile_options(Jolt INTERFACE -mf16c)
		endif()
		if (USE_FMADD AND NOT CROSS_PLATFORM_DETERMINISTIC)
			target_compile_options(Jolt INTERFACE -mfma)
		endif()

		if (NOT MSVC)
			target_compile_options(Jolt INTERFACE -mfpmath=sse)
		endif()
...
if (NOT ("${CMAKE_SYSTEM_NAME}" STREQUAL "Windows") AND NOT EMSCRIPTEN)
	target_compile_options(Jolt INTERFACE -pthread)
	target_link_options(Jolt INTERFACE -pthread)
endif()
...
if (EMSCRIPTEN)
	...
	target_link_options(Jolt INTERFACE -sSTACK_SIZE=1048576 -sINITIAL_MEMORY=134217728 -Wno-limited-postlink-optimizations)
endif()